...
import { generateWebsite } from '../services/campaignService.ts';
import { verifyEmailEligibility } from '../services/emailVerificationService.ts';
import { performSearchRound } from '../services/prospectorService.ts';
...
export default function Prospector() {
  ...
  const [currentStep, setCurrentStep] = useState<
    'idle' | 'searching' | 'scrolling' | 'extracting' | 'qualifying'
  >('idle');
  const [activeUrl, setActiveUrl] = useState('');
  const [lastScreenshot, setLastScreenshot] = useState<string | null>(null);
  const [logs, setLogs] = useState<
    { id: string; text: string; type: 'info' | 'success' | 'error' | 'warning' }[]
  >([]);
  ...
  useEffect(() => {
    (window as any).setCampaignActive?.('prospector-search', isSearching);
    return () => {
      (window as any).setCampaignActive?.('prospector-search', false);
    };
  }, [isSearching]);
  ...
  const startProspecting = async (e: React.FormEvent) => {
    ...
    setFoundLeads([]);
    setActiveUrl(
      `https://www.google.com/maps/search/${encodeURIComponent(niche)}+in+${encodeURIComponent(city)}`,
    );

    if (demoMode) {
      // --- Detailed Staggered Demo Mode Simulation ---
      addLog('[SYSTEM] Initializing Playwright stealth browser engine (DEMO MODE)...', 'info');
      await new Promise((r) => setTimeout(r, 1200));
      addLog(
        '[BROWSER] Rotating user-agent and applying fingerprint protection bypasses...',
        'info',
      );
      await new Promise((r) => setTimeout(r, 1000));
      addLog('[BROWSER] Connection established via secure residential gateway node.', 'success');
      await new Promise((r) => setTimeout(r, 800));
      addLog('[BROWSER] Navigating to Google Maps search terminal...', 'info');
      await new Promise((r) => setTimeout(r, 1500));

      setCurrentStep('scrolling');
      addLog(`[BROWSER] Executing Map search query: "${niche} in ${city}"`, 'info');
      await new Promise((r) => setTimeout(r, 1200));
      addLog('[SCROLLER] Scrolling viewport coordinate: Y=1800px (12 items loaded)', 'info');
      await new Promise((r) => setTimeout(r, 1200));
      addLog('[SCROLLER] Scrolling viewport coordinate: Y=4300px (28 items loaded)', 'info');
      await new Promise((r) => setTimeout(r, 1200));
      addLog('[SCROLLER] Scrolling viewport coordinate: Y=6900px (45 items loaded)', 'info');
      await new Promise((r) => setTimeout(r, 1000));
      addLog('[BROWSER] Infinite-scroll limit reached. 45 business listings detected.', 'success');
      await new Promise((r) => setTimeout(r, 1200));

      setCurrentStep('extracting');
      addLog('[Crawl4AI] Parsing DOM element clusters and extracting raw HTML blocks...', 'info');
      await new Promise((r) => setTimeout(r, 1500));
      addLog('[Crawl4AI] Semantic tag classification complete. Filtering candidates...', 'info');
      await new Promise((r) => setTimeout(r, 1200));

      const mockLeadDataList = [
        {
          name: `${city} Elite ${niche || 'Designs'}`,
          address: `104 N 12th St, ${city}`,
          phone: '(813) 555-0192',
          email: `contact@${city.toLowerCase().replace(/[^a-z0-9]/g, '')}elite${(niche || 'service').toLowerCase().replace(/[^a-z0-9]/g, '')}.com`,
        },
        {
          name: `Premier ${niche || 'Contractors'} & Co`,
          address: `501 Grand Ave, ${city}`,
          phone: '(813) 555-0248',
          email: `info@premier${(niche || 'contractors').toLowerCase().replace(/[^a-z0-9]/g, '')}.com`,
        },
      ];

      for (let i = 0; i < mockLeadDataList.length; i++) {
        const lead = mockLeadDataList[i];
        setCurrentStep('qualifying');
        addLog(`[ANALYZER] Deep inspecting business parameters for: "${lead.name}"`, 'info');
        await new Promise((r) => setTimeout(r, 1200));
        addLog(
          '[VERIFIER] Executing Reverse-DNS record checks on brand domain candidate...',
          'info',
        );
        await new Promise((r) => setTimeout(r, 1000));
        addLog(`[VERIFIER] Performing live SMTP mailbox handshake for "${lead.email}"...`, 'info');
        await new Promise((r) => setTimeout(r, 1200));
        addLog(
          '[SMTP] HELO/EHLO handshake success (Response: 250 OK). Mailbox is valid.',
          'success',
        );
        await new Promise((r) => setTimeout(r, 1000));
        addLog(
          `[QUALIFIER] CRITICAL FINDING: "${lead.name}" has no existing web presence (SOP criteria fully satisfied).`,
          'success',
        );
        await new Promise((r) => setTimeout(r, 1000));
        addLog(
          `[SYSTEM] Syncing qualified prospect metadata to Faiz's cloud repository...`,
          'info',
        );
        await new Promise((r) => setTimeout(r, 800));
        ...
        if (autoLaunch) {
          addLog(
            `[AI] Analyzing brand guidelines and niche layout options for "${lead.name}"...`,
            'info',
          );
          await new Promise((r) => setTimeout(r, 1200));
          const archetype = getBestArchetype(lead.name, niche);
          addLog(
            `[AI] Archetype matches: "${archetype.name}" (Design Style: ${archetype.id})`,
            'success',
          );
          await new Promise((r) => setTimeout(r, 1000));
          addLog('[AI] Compiling responsive single-page visual mockup structure...', 'info');
          await new Promise((r) => setTimeout(r, 1500));
          ...
          const simHtml = `
            ...
            <body className="bg-slate-900 text-slate-100 font-sans min-h-screen flex flex-col justify-between">
              <header className="p-6 border-b border-slate-800 flex justify-between items-center max-w-5xl mx-auto w-full">
                <span className="text-xl font-bold tracking-tight text-white">${lead.name}</span>
                <span className="px-4 py-1.5 bg-brand-500/10 text-brand-400 rounded-full text-xs font-semibold uppercase">${niche} Specialist</span>
              </header>
              <main className="max-w-3xl mx-auto text-center px-6 py-20 space-y-8">
                <h1 className="text-5xl font-black text-white leading-none">The Ultimate ${niche} Craftsmanship in ${city}.</h1>
                <p className="text-slate-400 text-lg">We deliver unparalleled precision, elegant designs, and lifetime durability. Contact our specialists today for a customized workspace design consultation.</p>
                <button className="bg-white text-slate-950 px-8 py-4 rounded-xl font-bold hover:bg-slate-200 transition-colors shadow-lg shadow-white/10">Get Free Estimate</button>
              </main>
              <footer className="p-6 border-t border-slate-800 text-center text-xs text-slate-500">
                © 2026 ${lead.name}. All rights reserved. ${lead.address}.
              </footer>
            </body>
            </html>
          `;

          leadData.generatedHtml = simHtml;
          leadData.designArchetype = archetype.id;
          ...
          addLog(
            '[AI] Mockup compiled successfully! Formulating personalized pitch narrative...',
            'success',
          );
          await new Promise((r) => setTimeout(r, 1200));
          addLog(
            `[MAILER] Triggering SMTP transaction channel to send proposal (Offer: €${setupPrice} + €${monthlyPrice}/mo)...`,
            'info',
          );
          await new Promise((r) => setTimeout(r, 1500));
          addLog('[MAILER] 250 OK: Outreach message delivered. Pitch active!', 'success');
          await new Promise((r) => setTimeout(r, 800));
        }
      ...
      setCurrentStep('idle');
      addLog(
        `[SYSTEM] Mission completed. Synchronized ${mockLeadDataList.length} qualified prospects to your database.`,
        'success',
      );
    ...
    try {
      ...
      addLog(
        `[Crawl4AI] Semantic extraction complete. Analyzing ${results.length} unique candidates...`,
        'info',
      );
      ...
      for (const lead of results) {
        ...
        const mapsUrl =
          lead.maps_url || `https://www.google.com/maps/search/${encodeURIComponent(businessName)}`;
        ...
        if (targetEmail) {
          ...
          if (!verification.isValid || verification.score < 40) {
            addLog(
              `[VERIFIER] Skipping ${businessName} - Email ineligible: ${verification.reason} (Score: ${verification.score})`,
              'warning',
            );
            continue;
          }
          addLog(
            `[VERIFIER] Email verified (Score: ${verification.score}). Proceeding...`,
            'success',
          );
        }
        ...
        try {
          ...
          const scrapeRes = await withRetry(() =>
            fetch('/api/scrape', {
              method: 'POST',
        ...
        const hasWebsite = Boolean(lead.Website || lead.website);

        if (hasWebsite) {
          addLog(
            `[QUALIFIER] DISCARDED: ${businessName} - Website detected (${lead.Website || lead.website}).`,
            'warning',
          );
          continue;
        }
        ...
        if (autoLaunch) {
          ...
          try {
            const { html, archetype: archetypeId } = await withRetry(() =>
              generateWebsite(businessName, niche, city),
            );
            ...
            addLog(
              `[AI] ${archetype.name} website generated. Outreach offer (€${setupPrice} + €${monthlyPrice}/mo) sent.`,
              'success',
            );
          } catch {
      ...
      console.error(error);
      addLog(
        `[CRITICAL] System Error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'error',
      );
      setCurrentStep('idle');
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 pb-20">
      {/* Header & Form */}
      <div className="glass p-10 rounded-2xl shadow-xl relative overflow-hidden border border-white/[0.03]">
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/[0.02] rounded-full blur-3xl -mr-32 -mt-32" />

        <div className="flex items-center gap-6 mb-10 relative z-10">
          <div className="w-14 h-14 bg-white/[0.03] rounded-xl flex items-center justify-center border border-white/[0.05] shadow-inner">
          ...
          <div>
            ...
            <p className="text-brand-500 font-medium mt-1">
              Specify a city and niche to start the autonomous hunt.
            </p>
          </div>
        </div>

        <form
          onSubmit={startProspecting}
          className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10"
        >
          <div className="space-y-3">
            <label className="text-[10px] font-bold text-brand-600 uppercase tracking-[0.2em] ml-1">
              Target City
            </label>
          ...
          <div className="space-y-3">
            ...
            <div className="relative group">
              ...
              <input
                ...
                placeholder="e.g. Bakery & Patisserie"
                className="w-full bg-brand-950/50 border border-white/[0.03] rounded-xl py-3.5 pl-12 pr-4 text-white focus:border-white/[0.1] focus:ring-4 focus:ring-white/[0.02] outline-none transition-all placeholder:text-brand-800 font-medium text-sm"
                disabled={isSearching}
              />
            </div>
          ...
          <div className="flex items-end gap-4">
            <div className="flex-[1.3] space-y-3">
              ...
              <button
                ...
                className={cn(
                  'w-full flex items-center justify-between px-3 sm:px-4 py-3.5 rounded-xl border transition-all duration-300 active:scale-[0.96]',
                  autoLaunch
                ...
                >
                  <div
                    className={cn(
                      ...
                      autoLaunch ? 'right-0.5 bg-brand-950' : 'left-0.5 bg-brand-400',
                    )}
                  />
                </div>
              </button>
            ...
            <button
              ...
              className="btn-lg btn-accent flex-1 flex items-center justify-center gap-2"
            >
              {isSearching ? (
                <>
                  <Loader2 className="w-5 h-5 animate-spin shrink-0" />
                  <span className="truncate">Hunting...</span>
                </>
              ) : (
                <>
                  <Search className="w-5 h-5 shrink-0" />
                  <span className="truncate">Start Hunt</span>
                </>
              )}
            </button>
          </div>
        ...
        {/* Quick Presets */}
        <div className="mt-6 pt-6 border-t border-white/[0.02] flex items-center gap-3 flex-wrap relative z-10">
          <span className="text-[10px] font-bold text-brand-600 uppercase tracking-widest mr-1">
            Quick Target Presets:
          </span>
          {[
            { city: 'Paris', niche: 'Bakery & Patisserie', emoji: '🥖' },
            { city: 'Lyon', niche: 'Artisan Bakery', emoji: '🥐' },
            { city: 'Nice', niche: 'French Bistro', emoji: '🍷' },
            { city: 'Toulouse', niche: 'Floral & Botanical', emoji: '🌸' },
          ].map((preset) => (
            <button
              key={preset.city + preset.niche}
              type="button"
              disabled={isSearching}
              onClick={() => {
                setCity(preset.city);
              ...
              className="text-xs bg-white/[0.03] hover:bg-white/[0.08] text-brand-300 hover:text-white px-3 py-1.5 rounded-lg border border-white/[0.05] transition-all flex items-center gap-1.5 font-medium"
            >
              <span>{preset.emoji}</span>
              <span>{preset.city} ({preset.niche})</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2 glass rounded-2xl overflow-hidden flex flex-col h-[650px] shadow-2xl border border-white/[0.03]">
          <div className="bg-white/[0.01] px-6 py-4 border-b border-white/[0.02] flex items-center gap-8">
            <div className="flex gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-brand-800" />
              <div className="w-2.5 h-2.5 rounded-full bg-brand-800" />
              <div className="w-2.5 h-2.5 rounded-full bg-brand-800" />
            </div>
            <div className="flex-1 bg-black/40 rounded-lg px-4 py-2 text-[11px] text-brand-600 font-mono truncate flex items-center gap-3 border border-white/[0.02]">
            ...
            <AnimatePresence mode="wait">
              {currentStep === 'idle' ? (
                ...
                >
                  ...
                  <p className="text-brand-700 font-medium tracking-tight text-sm">
                    ...
                    {lastScreenshot ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center space-y-8">
                          ...
                          <p className="text-brand-700 font-mono text-[10px] uppercase tracking-[0.4em] animate-pulse-subtle">
                        ...
                        <div className="flex items-center gap-6 glass-dark p-5 rounded-3xl min-w-[260px] shadow-2xl border border-white/[0.03]">
                          <div className="relative w-14 h-14 bg-white/[0.02] border border-white/[0.05] rounded-2xl flex items-center justify-center flex-shrink-0 shadow-inner">
                            {currentStep === 'searching' && (
                              <Search className="w-6 h-6 text-white animate-float" />
                            )}
                          ...
                          <div>
                            <h3 className="text-lg font-display font-bold text-white capitalize leading-none mb-2">
                              {currentStep}...
                            </h3>
                        ...
                        <div className="flex-1 glass-dark rounded-3xl p-5 font-mono text-[11px] text-brand-400 space-y-2 overflow-hidden h-[94px] shadow-2xl border border-white/[0.03]">
                          <div className="flex justify-between border-b border-white/[0.02] pb-3 mb-3">
                            <span className="flex items-center gap-2.5 opacity-40">
                              <Terminal className="w-4 h-4" /> HUD_CORE_V4.0
                            </span>
                            <span className="text-white flex items-center gap-2 font-bold opacity-80">
                              <ShieldCheck className="w-4 h-4" /> ENCRYPTED_SESSION
                            </span>
                          </div>
        ...
        <div className="flex flex-col gap-8">
          <div className="glass rounded-2xl overflow-hidden flex flex-col h-[650px] shadow-2xl border border-white/[0.03]">
            <div className="bg-white/[0.01] px-6 py-4 border-b border-white/[0.02] flex items-center justify-between">
              <div className="flex gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-brand-900" />
                <div className="w-2.5 h-2.5 rounded-full bg-brand-900" />
                <div className="w-2.5 h-2.5 rounded-full bg-brand-900" />
              </div>
              <span className="text-[10px] font-mono text-brand-600 uppercase tracking-[0.3em] font-bold">
                Autonomous_Terminal
              </span>
            </div>
            <div className="flex-1 p-6 font-mono text-[11px] overflow-y-auto space-y-4 scrollbar-hide">
              {logs.length === 0 && !isSearching && (
                <div className="flex flex-col items-center justify-center h-full text-brand-800 space-y-4 text-center px-4">
                  <Terminal className="w-10 h-10 opacity-20 text-brand-400" />
                  <div className="space-y-1">
                    <p className="font-mono text-xs text-brand-500 font-bold tracking-tight">
                      Awaiting command sequence...
                    </p>
                    <p className="text-[11px] text-brand-600 font-sans max-w-xs">
                      Enter a Target City & Niche above, or click a quick preset to launch the scraper.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      setCity('Paris');
                      setNiche('Bakery & Patisserie');
                      const form = document.querySelector('form');
                      if (form) form.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }));
                    }}
                    className="mt-2 text-xs bg-white/5 hover:bg-white/10 text-white font-mono px-4 py-2 rounded-xl border border-white/10 transition-all flex items-center gap-2"
                  >
                    <Zap className="w-3.5 h-3.5 text-amber-400" />
                    <span>Run Quick Sample Hunt</span>
                  </button>
                </div>
              )}
              {logs.map((log) => (
              ...
              {foundLeads
                ...
                .map((lead, i) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    className="glass p-5 rounded-xl border border-white/[0.03] flex flex-col gap-5 hover:border-white/[0.1] transition-all group"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={cn(
                          ...
                          lead.status === 'outreach_sent'
                            ...
                            : 'bg-brand-900/50 text-brand-500 border-white/[0.03]',
                        )}
                      >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-brand-600 truncate mt-1 font-mono uppercase tracking-widest">
                        {lead.city}
                      </p>
                    </div>
                    {lead.last_outreach_attempt && (
                      <div className="pt-4 border-t border-white/[0.02] flex items-center justify-between">
                        <span className="text-[9px] font-bold text-brand-700 uppercase tracking-widest">
                          Last Outreach
                        </span>
