...
import { analyzeAndReply, simulateIncomingReply } from '../services/negotiatorService.ts';
...
export default function Inbox() {
  ...
  const DEMO_NEGOTIATIONS = [
    {
      ...
      messages: [
        {
          sender: 'ai',
          text: 'Bonjour Pierre! We created a custom web design for Boutique Pâtisserie Paris. The setup is €349 with €19/mo hosting.',
          timestamp: new Date(Date.now() - 86400000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        },
        {
          sender: 'prospect',
          text: 'Bonjour! The preview looks fantastic. Is it possible to integrate our online wedding cake booking form and custom menu catalog?',
          timestamp: new Date(Date.now() - 43200000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        },
        {
          sender: 'ai',
          text: 'Absolutely! We can embed the custom booking form and live menu catalog at no extra cost. Would you like to launch the site this week?',
          timestamp: new Date(Date.now() - 21600000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        },
      ],
    },
    {
      ...
      messages: [
        {
          sender: 'ai',
          text: 'Bonjour! Here is your custom single-page website concept for Grand Café Marseille.',
          timestamp: new Date(Date.now() - 172800000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        },
        {
          sender: 'prospect',
          text: 'Looks great. What is the turn-around time once we send our photos?',
          timestamp: new Date(Date.now() - 86400000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        },
      ],
    },
  ...
  const effectiveLeads = demoMode
    ? [...leads, ...DEMO_NEGOTIATIONS.filter((dn) => !leads.some((l) => l.id === dn.id))]
    : leads;
  ...
  const handleManualReply = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!(selectedLead && replyText.trim()) || isSending) {
      return;
    }
    ...
    try {
      ...
      await addCampaignLog({
        ...
        status: 'info',
        message: `SDR sent manual reply: "${replyText}"`,
      });
      setReplyText('');
    } catch (error) {
  ...
  const handleSimulateReply = async (lead: any) => {
    try {
      ...
      await addCampaignLog({
        ...
        status: 'info',
        message: `Incoming reply from ${lead.businessName}: "${reply}"`,
      });
      ...
      await addCampaignLog({
        ...
        action: 'AI Response',
        status: 'success',
        message: `AI Negotiator analyzed prospect reply and dispatched follow-up response.`,
      });
    } catch (err: any) {
      await addCampaignLog({
        leadId: lead.id,
        ...
        action: 'Negotiation',
        status: 'error',
        message: `Simulation / analysis failed: ${err?.message || String(err)}`,
      });
    }
  };
  ...
  const closeDeal = async (lead: any) => {
    try {
      ...
      await addCampaignLog({
        ...
        action: 'Lead Handoff',
        status: 'success',
        message: `Deal closed successfully! Handoff completed. Customer ready for delivery onboarding.`,
      });
  ...
  useEffect(() => {
    if (!selectedLead && effectiveLeads.length > 0) {
      setSelectedLead(effectiveLeads[0]);
    }
  }, [effectiveLeads, selectedLead]);
          ...
          {effectiveLeads.length === 0 ? (
            ...
            effectiveLeads.map((lead) => (
              <div
                ...
                className={cn(
                  ...
                  selectedLead?.id === lead.id
                    ? 'bg-white/[0.03] border-l-4 border-brand-400'
              ...
              >
                ...
                <p className="text-[10px] font-mono text-brand-600 uppercase tracking-widest mb-3">
                  {lead.niche}
                </p>
                <div className="flex items-center justify-between">
                  ...
                  {lead.messages?.length === 0 && lead.status === 'outreach_sent' && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
          ...
          <>
              ...
              <div className="flex items-center gap-4">
                <div className="text-right mr-4">
                  ...
                  <p className="text-xl font-display font-bold text-white tracking-tight">
                    €{selectedLead.offerPrice} + €{selectedLead.subscriptionPrice}/mo
                  </p>
                </div>
                <button onClick={() => closeDeal(selectedLead)} className="btn-md btn-accent px-8">
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8 scrollbar-hide">
                ...
                <div className="max-w-[80%] space-y-3">
                  <div className="flex items-center gap-3 text-brand-600 mb-1">
                    ...
                    <span className="text-[10px] font-mono">
                      {selectedLead.outreachSentAt?.toDate?.()?.toLocaleTimeString() || 'Sent'}
                    </span>
                  </div>
                  <div className="glass-dark p-6 rounded-3xl rounded-tl-none border border-white/[0.03] text-brand-200 text-sm leading-relaxed">
                    Hello {selectedLead.businessName} team, we've built a custom website mockup for
                    you. It's ready to launch for just €{selectedLead.offerPrice} setup and €
                    {selectedLead.subscriptionPrice}/mo. Would you like to see it?
                  </div>
                </div>
              </div>
              ...
              {selectedLead.messages?.map((msg: any, i: number) => (
                ...
                >
                  <div
                    className={cn('max-w-[80%] space-y-3', msg.role === 'user' ? '' : 'text-right')}
                  >
                    <div
                      className={cn(
                        'flex items-center gap-3 text-brand-600 mb-1',
